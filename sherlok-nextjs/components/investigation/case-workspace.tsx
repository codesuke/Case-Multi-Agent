...
export function CaseWorkspace({ investigationId, view }: { investigationId: string; view: View }) {
  return <CaseWorkspaceForInvestigation key={investigationId} investigationId={investigationId} view={view} />;
}

function CaseWorkspaceForInvestigation({ investigationId, view }: { investigationId: string; view: View }) {
...
function Verdict({ caseFile, investigationId, isComplete, onSnapshot }: { caseFile: CaseFile; investigationId: string; isComplete: boolean; onSnapshot: (snapshot: Snapshot) => void }) {
  ...
  if (!verdict) return <EmptyVerdict isComplete={isComplete} />;

  async function command(path: "decision" | "reinvestigation", body: object) {
    setBusy(true);
    setMessage(null);
    try {
      const response = await fetch(`/api/investigations/${encodeURIComponent(investigationId)}/${path}`, {
        method: "POST",
      ...
      const snapshot = asInvestigationSnapshot(result);
      if (!snapshot) throw new Error("The returned case file was incompatible. Refresh and try again.");
      onSnapshot(snapshot);
      if (path === "reinvestigation") {
        router.push(`/agent-workspace?investigation_id=${encodeURIComponent(investigationId)}`);
        return;
      }
      setMessage("Human decision recorded.");
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "The command could not be recorded.");
    } finally {
      setBusy(false);
    }
  }
...
function EmptyVerdict({ isComplete }: { isComplete: boolean }) {
  return isComplete
    ? <Notice title="No proposed Verdict" message="The investigation completed without a proposed Verdict." />
    : <Notice title="Verdict pending" message="The Lead Detective has not submitted a proposed Verdict yet." />;
}
