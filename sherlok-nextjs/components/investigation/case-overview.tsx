...
import { ArrowRight, Bot, ChevronRight, ClipboardList, FileText, FolderOpen, Lightbulb, Scale, ShieldAlert, Timer, UsersRound, Waypoints } from "lucide-react";
...
export function CaseOverview({ caseFile, investigationId, isComplete }: { caseFile: CaseFile; investigationId: string; isComplete: boolean }) {
  ...
  return <main className="min-h-[100dvh] bg-[#24160e] text-[#f8ebd2]">
    ...
    <div className="relative grid min-h-[100dvh] lg:grid-cols-[225px_minmax(0,1fr)]">
      ...
      <div className="min-w-0"><header className="flex h-[79px] items-center border-b-[3px] border-[#d8c4a0] bg-[#24160e]/95 px-6"><p className="ml-auto flex items-center gap-3 text-sm font-semibold text-[#f8ebd2]" role="status"><span className={`size-3 rounded-full ${data.isReview ? "bg-[#f4b941]" : "bg-[#18afa3]"} shadow-[0_0_0_4px_rgba(24,175,163,.12)]`} /><span>{data.statusLabel}</span></p></header>
        <section className="bg-[#f4e8d0] px-6 py-6 text-[#1e2831] lg:min-h-[calc(100dvh-79px)]"><div className="mx-auto max-w-[1320px]"><div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_376px]"><div className="min-w-0"><header className="mb-7"><p className="eyebrow text-[#875e26]">Active investigation</p><h1 className="mt-2 font-serif text-[46px] font-semibold leading-[.94] tracking-tight text-[#121b25]">Case overview</h1><p className="mt-2 font-serif text-xl leading-7 text-[#514e4b]">High-level view of the current Case File and its key materials.</p></header><InvestigationStatus data={data} href={href(data.actionRoute)} /><div className="my-8"><SourceMaterial caseFile={caseFile} href={href("overview")} /></div><CanonicalMaterial caseFile={caseFile} /></div>
          <aside className="space-y-3 border-[#d8c4a0] xl:border-l xl:pl-4"><Summary href={href("overview")} icon={FileText} title="Sources" value={data.sourceCount === 0 ? (isComplete ? "None" : "Waiting") : String(data.sourceCount)} description={data.sourceCount === 0 ? (isComplete ? "No canonical source blocks are available" : "Waiting for material preparation") : "Documents and data sources ingested"} /><Summary href={href("evidence")} icon={ClipboardList} title="Evidence" value={data.evidenceCount === 0 ? (isComplete ? "None" : "Waiting") : String(data.evidenceCount)} description={data.evidenceCount === 0 ? (isComplete ? "No Evidence items were reported" : "Waiting for collection") : "Evidence items extracted and categorized"} /><Summary href={href("timeline")} icon={Timer} title="Timeline issues" value={data.timelineIssues === 0 ? (isComplete ? "0" : "Waiting") : String(data.timelineIssues)} description={data.timelineIssues === 0 ? (isComplete ? "No open timeline issues were reported" : "Waiting for timeline reconciliation") : "Potential inconsistencies identified"} /><NextStep data={data} href={href(data.actionRoute)} /></aside>
        </div></div></section></div>
    </div></main>;
}

function InvestigationStatus({ data, href }: { data: OverviewData; href: string }) { return <section className="flex gap-7 rounded-lg border border-[#e6a522] bg-[#fbf2de] p-6 sm:items-center"><span className="grid size-[88px] shrink-0 place-items-center rounded-full bg-[#f6deaa] text-[#121b25]"><UsersRound className="size-10" strokeWidth={1.5} /></span><div><h2 className="font-serif text-[30px] font-semibold leading-tight text-[#121b25]">{data.statusTitle}</h2><p className="mt-1 max-w-2xl text-[17px] leading-6 text-[#3e4650]">{data.statusDescription}</p><Link href={href} className="mt-4 inline-flex items-center gap-2 text-sm font-bold text-[#006b54] hover:underline">{data.actionLabel}<ArrowRight className="size-4" /></Link></div></section>; }
...
function overviewData(caseFile: CaseFile, isComplete: boolean) {
  const status = caseFile.verdict?.review_status;
  const isReview = status === "awaiting_review" || status === "accepted" || status === "rejected";
  const statusTitle = status === "awaiting_review"
    ? "A proposed Verdict is ready for review"
    : status === "accepted"
      ? "Human decision: accepted"
      : status === "rejected"
        ? "Human decision: rejected"
        : status === "reinvestigation_requested"
          ? "Re-investigation in progress"
          : isComplete
            ? "Investigation complete"
            : "Investigation in progress";
  const statusDescription = isReview
    ? "Review the evidence-cited proposal and its Human Decision status."
    : "Open the Agent Workspace to inspect current workflow state and Investigation Events.";
  const evidenceCount = caseFile.evidence?.length ?? 0;
  ...
  return {
    ...
    statusTitle,
    statusDescription,
    actionLabel: isReview ? "Review proposed Verdict" : "Open Agent Workspace",
    actionRoute: isReview ? "verdict" : "agents",
    evidenceCount,
    sourceCount: sourceSummaries(caseFile).length,
    timelineIssues: caseFile.timeline?.issues?.length ?? 0,
    nextStep: isReview
      ? "Review the proposed Verdict and its cited Evidence before recording a Human Decision."
      : "Open the Agent Workspace for current workflow state and Investigation Events.",
  };
}
